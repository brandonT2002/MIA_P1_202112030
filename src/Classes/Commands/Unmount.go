...
func (u *Unmount) unmount() {
	...
	// if env.Disks[string(driveletter)] != nil {
	// 	return
	// }
	if disk, ok := env.Disks[driveletter]; ok {
		...
		if disk.Ids[u.Params["id"]] != nil {
			...
			for i := 0; i < len(mbr.Partitions); i++ {
				if mbr.Partitions[i].Status == '0' && u.compareStrings(mbr.Partitions[i].Name, namePartition) {
			...
			u.printError(fmt.Sprintf("\033[33mError unmount: Intenta desmontar una partición desmontada o inexistente en el disco \"%s\"\033[0m", driveletter))
			return
		}
		u.printError(fmt.Sprintf("\033[33mError unmount: No existe el código de partición \"%s\" para desmontar en el disco \"%s\"\033[0m", u.Params["id"], driveletter))
		return
	}
	u.printError(fmt.Sprintf("\033[33mError unmount: No existe el disco \"%s\" para desmontar la partición\033[0m", driveletter))
}
